...
const PEOPLE_CHUNKS = 8, EDGE_CHUNKS = 4, PHOTO_CHUNKS = 24;
const chunkOf = (id, n) => {
...
const hijriOf = (v) => {
  ...
  try {
    ...
    const opts = m[3] ? { day: "numeric", month: "long", year: "numeric" }
      : m[2] ? { month: "long", year: "numeric" } : { year: "numeric" };
...
const mailLink = (v) => { v = String(v || "").trim(); return v && v.includes("@") ? `mailto:${v}` : null; };
...
function FamilyTree() {
  ...
  const [showWomenKids, setShowWomenKidsRaw] = useState(() => {
    try { return localStorage.getItem("ft:womenkids") === "1"; } catch { return false; }
  });
  const setShowWomenKids = (v) => { setShowWomenKidsRaw(v); try { localStorage.setItem("ft:womenkids", v ? "1" : "0"); } catch {} };
  ...
  useEffect(() => {
    ...
    const un = FB.db.collection("tree").onSnapshot(snap => {
      let meta = null; const pplMap = {}, edgMap = {};
      snap.forEach(doc => {
        if (doc.id === "meta") { meta = doc.data(); return; }
        if (doc.id.startsWith("people-")) Object.assign(pplMap, doc.data());
        else if (doc.id.startsWith("edges-")) Object.assign(edgMap, doc.data());
      });
      if (!meta) {
      ...
      const ppl = Object.values(pplMap)
        .sort((a, b) => ((a.g ?? 99) - (b.g ?? 99)) || (a.y - b.y) || (a.x - b.x));
      setPeople(ppl); setEdges(Object.values(edgMap));
      setCloud("live"); setLoading(false); loadedRef.current = true;
  ...
  useEffect(() => {
    ...
    const un = FB.db.collection("photos").onSnapshot(snap => {
      const ph = {}; snap.forEach(d => Object.assign(ph, d.data()));
      setPhotos(ph); setPhotosReady(true);
    }, () => {});
    return un;
  }, [cloudLive]);

  // ---------- Cloud: auth / history ----------
  useEffect(() => { if (FB) return FB.auth.onAuthStateChanged(u => setUser(u)); }, []);
  useEffect(() => {
    if (!FB || !cloudLive) return;
    return FB.db.collection("revisions").orderBy("ts", "desc").limit(100)
      .onSnapshot(s => {
        const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setRevisions(a);
      }, () => {});
  }, [cloudLive]);
  ...
  useEffect(() => {
    ...
    fetch("./family-tree-photos.json")
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (!d || !d.photos) return;
        setPhotos(prev => ({ ...d.photos, ...prev }));
        setPhotosReady(true);
      })
      .catch(() => {});
  }, [cloud]);
  ...
  useEffect(() => {
    ...
    const r = canvasRef.current?.getBoundingClientRect();
    const vw0 = r ? r.width : 900, vh0 = r ? r.height : 600;
    const n0 = people[0] || { x: 0, y: 120 };
    const s0 = vw0 < 700 ? 1.1 : 0.62;
    setView({ s: s0, tx: vw0 / 2 - n0.x * s0, ty: Math.max(24, vh0 * 0.16) - n0.y * s0 });
  ...
  const cloudWritePeople = (persons) => {
    ...
    const byChunk = {};
    persons.forEach(p => { (byChunk[chunkOf(p.id, PEOPLE_CHUNKS)] ||= {})[p.id] = p; });
    const batch = FB.db.batch();
    Object.entries(byChunk).forEach(([c, m]) => batch.set(FB.db.doc("tree/people-" + c), m, { merge: true }));
    return batch.commit().catch(e => window.alert("تعذر الحفظ: " + (e.message || "")));
  };
  const cloudWriteEdges = (es) => {
    if (!cloudLive) return Promise.resolve();
    const byChunk = {};
    es.forEach(e => { (byChunk[chunkOf(e.id, EDGE_CHUNKS)] ||= {})[e.id] = e; });
    const batch = FB.db.batch();
    Object.entries(byChunk).forEach(([c, m]) => batch.set(FB.db.doc("tree/edges-" + c), m, { merge: true }));
    return batch.commit().catch(e => window.alert("تعذر الحفظ: " + (e.message || "")));
  };
  const cloudDeleteEdges = (ids) => {
    if (!cloudLive || !ids.length) return Promise.resolve();
    const batch = FB.db.batch();
    ids.forEach(eid => batch.update(FB.db.doc("tree/edges-" + chunkOf(eid, EDGE_CHUNKS)), { [eid]: FV().delete() }));
    return batch.commit().catch(() => {});
  };
  const cloudRevision = (text, extra) => {
    if (!cloudLive) return;
    FB.db.collection("revisions").add({
      ts: Date.now(), uid: user?.uid || "", name: user?.displayName || user?.email || "",
      text, ...(extra || {}),
    }).catch(() => {});
  };
  ...
  const signIn = async () => {
    ...
    catch (e) { if (e?.code !== "auth/popup-closed-by-user") window.alert("تعذر تسجيل الدخول: " + (e.message || "")); }
  };
  ...
  const seedToCloud = async (p, e, ph) => {
    ...
    try {
      const pc = {}, ec = {}, phc = {};
      p.forEach(x => { (pc[chunkOf(x.id, PEOPLE_CHUNKS)] ||= {})[x.id] = x; });
      e.forEach(x => { (ec[chunkOf(x.id, EDGE_CHUNKS)] ||= {})[x.id] = x; });
      Object.entries(ph || {}).forEach(([id, v]) => { (phc[chunkOf(id, PHOTO_CHUNKS)] ||= {})[id] = v; });
      const batch = FB.db.batch();
      ...
      await FB.db.doc("tree/meta").set({ seeded: true, ts: Date.now(), version: SEED.version || 2 });
      FB.db.collection("revisions").add({
        ts: Date.now(), uid: user?.uid || "", name: user?.displayName || "",
        text: `رفع الشجرة إلى السحابة (${p.length} فرد، ${Object.keys(ph || {}).length} صورة)`,
      }).catch(() => {});
    } catch (err) { window.alert("فشل الرفع: " + (err.message || "")); }
    setBusyMsg("");
  };
  ...
  const revertRevision = async (rev) => {
    ...
    try {
      if (rev.kind === "update" && rev.personId && rev.before) {
        ...
        await FB.db.doc("tree/people-" + chunkOf(rev.personId, PEOPLE_CHUNKS)).update({ [rev.personId]: FV().delete() });
        await cloudDeleteEdges(eids);
      } else if (rev.kind === "delete" && rev.before?.person) {
        const pr = rev.before.person, es = rev.before.edges || [];
    ...
    } catch (e) { window.alert("تعذر التراجع: " + (e.message || "")); }
  };
  ...
  const addPerson = (preset = {}) => {
    ...
    cloudWritePeople([np]);
    cloudRevision(`إضافة شخص جديد: ${np.name}`, { kind: "add", personId: np.id });
    return np;
  };
  ...
  const updatePerson = (id, patch, logText) => {
    ...
    if (!cur) return;
    const slot = (dirtyRef.current[id] ||= { before: { ...cur }, next: { ...cur } });
    slot.next = { ...slot.next, ...patch };
    ...
    slot.timer = setTimeout(() => {
      ...
      cloudRevision(`تعديل بيانات ${full.name}`, {
        kind: "update", personId: id, before: pick(slot.before, keys), after: pick(full, keys),
      });
    }, 1200);
  };
  ...
  const deletePerson = (id) => {
    ...
    if (cloudLive) {
      FB.db.doc("tree/people-" + chunkOf(id, PEOPLE_CHUNKS)).update({ [id]: FV().delete() }).catch(() => {});
      cloudDeleteEdges(removedEdges.map(e => e.id));
      FB.db.doc("photos/c" + chunkOf(id, PHOTO_CHUNKS)).set({ [id]: FV().delete() }, { merge: true }).catch(() => {});
      cloudRevision(`حذف الشخص: ${p.name}`, { kind: "delete", personId: id, before: { person: p, edges: removedEdges } });
    }
  };
  ...
  const addEdge = (from, to, type) => {
    ...
    cloudRevision(txt, { kind: "edge-add", after: { edge } });
  };
  ...
  const removeEdge = (eid) => {
    ...
    cloudRevision(`حذف رابط: ${a} ← ${b}`, { kind: "edge-del", before: { edge: e } });
  };
  ...
  const addChild = (parentId, gender = "m") => {
    ...
    const wouldHide = parent.gender === "f"
      ? !!primParent[parent.id]
      : (viaWomen.has(parentId)
        || (!primParent[parentId] && spousesOf(parentId).some(sp => primParent[sp])));
    if (wouldHide && !showWomenKids) setShowWomenKids(true);
    const sibs = edges.filter(e => e.type !== "spouse" && e.from === parentId)
      .map(e => people.find(p => p.id === e.to)).filter(Boolean);
    ...
    cloudWriteEdges([edge]);
    cloudRevision(`ربط ${gender === "f" ? "بنت جديدة" : "ابن جديد"} بـ ${parent.name}`, { kind: "edge-add", after: { edge } });
  };
  ...
  const addParent = (childId, gender = "m") => {
    ...
    const parent = addPerson({
      x: freeX(child.x - 30, child.y - 250), y: child.y - 250,
      name: gender === "f" ? "والدة" : "والد",
      gender, g: child.g != null ? child.g - 1 : undefined,
    });
    const edge = { id: uid(), from: parent.id, to: childId };
    setEdges(prev => [...prev, edge]);
    addLog(`ربط ${gender === "f" ? "والدة" : "والد"} بـ ${child.name}`);
    cloudWriteEdges([edge]);
    cloudRevision(`ربط ${gender === "f" ? "والدة" : "والد"} بـ ${child.name}`, { kind: "edge-add", after: { edge } });
  };
  ...
  const addSpouse = (personId) => {
    ...
    const sp = addPerson({
      x: freeX(p0.x + NODE_W + 40, p0.y), y: p0.y,
      name: gender === "f" ? "زوجة" : "زوج",
      gender, g: p0.g,
    });
    const edge = { id: uid(), from: personId, to: sp.id, type: "spouse" };
    setEdges(prev => [...prev, edge]);
    addLog(`ربط ${gender === "f" ? "زوجة" : "زوج"} بـ ${p0.name}`);
    cloudWriteEdges([edge]);
    cloudRevision(`ربط ${gender === "f" ? "زوجة" : "زوج"} بـ ${p0.name}`, { kind: "edge-add", after: { edge } });
  };
  ...
  const handlePhoto = async (id, file) => {
    ...
    try {
      ...
      if (cloudLive) {
        await FB.db.doc("photos/c" + chunkOf(id, PHOTO_CHUNKS)).set({ [id]: data }, { merge: true });
        cloudRevision(`رفع صورة لـ ${nm}`, { kind: "photo", personId: id });
      } else {
  ...
  const removePhoto = (id) => {
    ...
    if (cloudLive) {
      FB.db.doc("photos/c" + chunkOf(id, PHOTO_CHUNKS)).set({ [id]: FV().delete() }, { merge: true }).catch(() => {});
      cloudRevision(`حذف صورة: ${nm}`, { kind: "photo", personId: id });
    } else {
      store.del(photoKey(id));
    }
    addLog(`حذف صورة: ${nm}`);
  ...
  {
    const pbc = {}, byId = {}, prim = {}, cm = {};
    edges.filter(e => e.type !== "spouse").forEach(e => (pbc[e.to] ||= []).push(e.from));
    people.forEach(p => { byId[p.id] = p; });
    for (const c in pbc) {
      const f = pbc[c].find(x => byId[x]?.gender !== "f") || pbc[c][0];
      prim[c] = f; (cm[f] ||= []).push(c);
    }
    const seeds = [];
    people.forEach(p => {
      ...
      if (p.gender !== "f" && !prim[p.id]) {
        ...
        const spIds = edges.filter(e => e.type === "spouse" && (e.from === p.id || e.to === p.id))
  ...
  const visPeople = people.filter(p =>
    (familyMode || p.gender !== "f") && (showWomenKids || !viaWomen.has(p.id)));
  ...
  const vw = cRect ? cRect.width : 1200, vh = cRect ? cRect.height : 800;
  const MARGIN = 600;
  const wx0 = (-view.tx) / view.s - MARGIN, wx1 = (vw - view.tx) / view.s + MARGIN;
  const wy0 = (-view.ty) / view.s - MARGIN, wy1 = (vh - view.ty) / view.s + MARGIN;
    ...
    <div dir="rtl" style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: C.sub }}>
              {visPeople.length} فرد · {visEdges.length} رابط{cloudLive ? " · ☁ مباشر" : ""}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {FB && cloud !== "off" && (user ? (
            <Btn C={C} onClick={signOut} title={user.email}>
              {(user.displayName || user.email || "").split(" ")[0]} · خروج
            </Btn>
          ) : (
            <Btn C={C} onClick={signIn}>تسجيل الدخول</Btn>
          ))}
          <Btn C={C} active={familyMode} onClick={() => familyMode ? lockFamily() : setShowUnlock(true)}>
            {familyMode ? "🔓 الوضع العائلي" : "🔒 الوضع العائلي"}
          </Btn>
          {(viaWomen.size > 0 || showWomenKids) && (
            <Btn C={C} active={showWomenKids} onClick={() => setShowWomenKids(!showWomenKids)}>
              👶 أبناء البنات{showWomenKids ? "" : ` (${viaWomen.size})`}
            </Btn>
          )}
          {canEdit && <Btn C={C} onClick={() => addPerson()}>+ شخص</Btn>}
          {canEdit && (
          ...
          <Btn C={C} onClick={() => setShowStats(true)}>📊 إحصائيات</Btn>
          <Btn C={C} onClick={() => setShowGallery(true)}>🖼️ معرض</Btn>
          <Btn C={C} onClick={exportData}>تصدير</Btn>
          {(!cloudLive || isAdmin) && <Btn C={C} onClick={() => importRef.current?.click()}>استيراد</Btn>}
      ...
      {cloud === "empty" && isAdmin && (
        <div style={{
          ...
          padding: "8px 14px", background: "#e8f2ee", borderBottom: "1px solid #bcd8cd",
          fontSize: 13, color: "#1f5c46", flexShrink: 0, zIndex: 29,
        }}>
          قاعدة البيانات السحابية جاهزة لكنها فارغة — ارفع الشجرة الحالية لتصبح مشتركة ومباشرة للجميع.
          <button onClick={seedCloudFromCurrent} disabled={!photosReady || !!busyMsg} style={{
            background: photosReady && !busyMsg ? C.gold : "#9db3aa", color: "#fff", border: "none",
            padding: "5px 14px", borderRadius: 6, cursor: photosReady && !busyMsg ? "pointer" : "wait",
            fontFamily: "'Tajawal'", fontSize: 13, fontWeight: 600,
          }}>{busyMsg || (photosReady ? "رفع البيانات إلى السحابة" : "بانتظار تحميل الصور…")}</button>
        </div>
      )}
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
        ...
        >
          ...
          <div style={{
            ...
            <svg style={{ position: "absolute", overflow: "visible", width: 1, height: 1, pointerEvents: "none" }}>
              {(() => {
                ...
                Object.keys(kids).forEach(pid => {
                  ...
                  const gx0 = Math.min(par.x, ...cs.map(c => c.x)), gx1 = Math.max(par.x, ...cs.map(c => c.x));
                  const gy0 = Math.min(par.y, ...cs.map(c => c.y)), gy1 = Math.max(par.y, ...cs.map(c => c.y));
            ...
            {/* nodes (virtualized: only those near the viewport) */}
            {drawnPeople.map(p => {
              ...
              return (
                <div key={p.id}
                  ...
                  {p.deceased && (
                    <div style={{
                      position: "absolute", top: 5, left: 5, background: "rgba(122,106,71,.13)",
                      color: "#7a6a47", fontSize: 9, padding: "2px 7px", borderRadius: 7,
                      fontFamily: "'Tajawal'", fontWeight: 500, whiteSpace: "nowrap",
                    }}>{mercy(p)}</div>
                  )}
                  <div style={{
                    fontFamily: "'Amiri', serif", fontWeight: 700, fontSize: 14.5, color: "#2c2415",
                    marginTop: 5, textAlign: "center", lineHeight: 1.3, maxWidth: "100%",
                    overflow: "hidden", display: "-webkit-box",
                    WebkitLineClamp: p.nickname ? 1 : 2, WebkitBoxOrient: "vertical",
                  }}>{p.name}</div>
                  {p.nickname && (
            ...
            {searchHits.length > 0 && (
              <div style={{
                ...
                {searchHits.slice(0, 30).map(p => {
                  const anc = [];
                  let c = p.id, n = 0;
                  while (primParent[c] && n++ < 4) { c = primParent[c]; if (pmap[c]) anc.push(pmap[c].name); }
                  const husband = !anc.length && spousesOf(p.id).map(sp => pmap[sp]?.name).filter(Boolean)[0];
                  return (
                    <div key={p.id} onClick={() => focusPerson(p)} style={{
                      padding: "7px 12px", cursor: "pointer", borderBottom: `1px solid ${C.border}`,
                    }}>
                      <div style={{ fontFamily: "'Amiri',serif", fontWeight: 700, fontSize: 14.5, color: C.parch }}>
                        {p.name}{p.nickname ? ` (${p.nickname})` : ""}{p.deceased ? ` · ${mercy(p)}` : ""}
                      </div>
                      {(anc.length > 0 || husband) && (
        ...
        {showPanel && (
          <div style={isMobile ? {
            ...
            background: C.panel, borderTop: `1px solid ${C.border}`, borderRadius: "16px 16px 0 0",
            ...
            {isMobile && (
              <div onClick={() => setShowPanel(false)}
                style={{ display: "grid", placeItems: "center", padding: "9px 0 4px", cursor: "pointer", flexShrink: 0 }}>
                <div style={{ width: 44, height: 5, borderRadius: 3, background: C.border }} />
              </div>
            )}
            <div style={{ display: "flex", borderBottom: `1px solid ${C.border}` }}>
              {[
                ["view", "الملف"], ["edit", "تحرير"],
                ["log", `السجل (${cloudLive ? revisions.length : log.length})`],
              ].map(([k, lbl]) => (
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
              ...
              {tab === "edit" && cloudLive && !isEditor && (
                <AuthGate C={C} onSignIn={signIn} />
              )}
              {tab === "edit" && canEdit && (
                selected ? (
              ...
              {tab === "log" && cloudLive && (
                <div>
                  {revisions.length === 0 && <div style={{ color: C.sub, fontSize: 14 }}>لا توجد تعديلات بعد.</div>}
                  {revisions.map(r => (
                    <div key={r.id} style={{ padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
                      <div style={{ fontSize: 14, color: C.parch }}>{r.text}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 2 }}>
                        <div style={{ fontSize: 11, color: C.sub }}>
                          {r.name ? r.name + " · " : ""}{fmtTime(r.ts)}
                        </div>
                        {isEditor && (r.before || r.kind === "add") && (
                          <button onClick={() => revertRevision(r)} style={{
                            background: "transparent", border: `1px solid ${C.border}`, color: C.sub,
                            borderRadius: 6, cursor: "pointer", fontSize: 11, fontFamily: "'Tajawal'", padding: "2px 8px",
                          }}>تراجع</button>
                        )}
                      </div>
                    </div>
      ...
      {showStats && (
        <StatsOverlay C={C} people={visPeople} edges={visEdges} photos={photos}
          familyMode={familyMode} onClose={() => setShowStats(false)} />
      )}
      {showGallery && (
        <GalleryOverlay C={C} people={visPeople} photos={photos}
          onClose={() => setShowGallery(false)}
          onOpen={(p) => { setShowGallery(false); focusPerson(p); }} />
      )}

      {busyMsg && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(43,42,37,.5)", zIndex: 90, display: "grid", placeItems: "center" }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: "20px 34px", fontFamily: "'Amiri',serif", fontSize: 18, color: C.gold }}>
            {busyMsg}
          </div>
        </div>
      )}
...
function ContactLinks({ C, person }) {
  ...
  const links = [
    ...
    ["البريد", mailLink(c.email), "#8a6d3b"],
...
function PhotoLightbox({ photo, name, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      position: "fixed", inset: 0, background: "rgba(20,18,14,.9)", zIndex: 120,
      ...
      <div style={{ textAlign: "center", maxWidth: "94vw" }}>
        <img src={photo} alt={name}
          ...
          style={{ maxWidth: "94vw", maxHeight: "80vh", borderRadius: 10, boxShadow: "0 20px 60px rgba(0,0,0,.55)" }} />
        <div style={{ color: "#f0ece2", fontFamily: "'Tajawal'", fontSize: 15, marginTop: 12, fontWeight: 600 }}>
          {name}{dim ? ` — الأبعاد: ${dim.w} × ${dim.h} بكسل` : ""}
        </div>
        <div style={{ color: "#b7ae9d", fontFamily: "'Tajawal'", fontSize: 12, marginTop: 4 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 6 }}>
        {photo ? (
          <div onClick={() => setZoomPhoto(true)} title="اضغط لعرض الصورة بالحجم الكامل"
            style={{ cursor: "zoom-in", textAlign: "center", marginBottom: 10 }}>
            <img src={photo} alt={`صورة ${person.name}`} style={{
              maxWidth: "100%", maxHeight: 190, objectFit: "contain", display: "block",
              borderRadius: 12, border: `3px solid ${person.gender === "f" ? "#b06a84" : C.gold}`,
              boxShadow: "0 4px 14px rgba(0,0,0,.12)", margin: "0 auto",
            }} />
            <div style={{ fontSize: 11, color: C.sub, marginTop: 5 }}>🔍 اضغط للتكبير</div>
          </div>
        ) : (
          <div style={{
        ...
        {zoomPhoto && photo && (
          <PhotoLightbox photo={photo} name={person.name} onClose={() => setZoomPhoto(false)} />
        )}
      ...
      {familyMode ? (
        <div style={{ fontSize: 14, lineHeight: 1.9 }}>
          {person.dob
            ? <>تاريخ الميلاد: {fmtDate(person.dob)}{hijriOf(person.dob) ? ` (${hijriOf(person.dob)} تقريبًا)` : ""}{age != null && !person.deceased ? ` · العمر ${age} سنة تقريبًا` : ""}</>
            : <span style={{ color: C.sub, fontSize: 13 }}>لم يُسجل تاريخ الميلاد.</span>}
          {person.deceased && person.dod && <div>سنة الوفاة: {yearOf(person.dod)}{hijriOf(person.dod) ? ` (${hijriOf(person.dod)} تقريبًا)` : ""}{age != null ? ` · عن عمر ${age} سنة تقريبًا` : ""}</div>}
        </div>
      ) : (
      ...
      <ShareButton C={C} person={person} />
    </div>
  );
}

function ShareButton({ C, person }) {
  ...
  return (
    <button onClick={share} style={{
      width: "100%", marginTop: 8, padding: "9px", borderRadius: 7,
      border: `1px solid ${C.border}`, background: "transparent", color: copied ? C.gold : C.sub,
      cursor: "pointer", fontFamily: "'Tajawal'", fontSize: 13, fontWeight: 600,
    }}>{copied ? "✓ تم نسخ الرابط" : "🔗 نسخ رابط هذا الشخص"}</button>
  );
}

function Minimap({ C, people, view, vw, vh, onJump }) {
  const ref = useRef(null);
  const boundsRef = useRef(null);
  const MW = 220, MH = 110;

  useEffect(() => {
    const cv = ref.current; if (!cv || !people.length) return;
    const xs = people.map(p => p.x), ys = people.map(p => p.y);
    const minX = Math.min(...xs) - 800, maxX = Math.max(...xs) + 800;
    const minY = Math.min(...ys) - 400, maxY = Math.max(...ys) + 400;
    const sx = MW / (maxX - minX), sy = MH / (maxY - minY);
    boundsRef.current = { minX, minY, sx, sy };
    ...
    const rx = ((-view.tx) / view.s - minX) * sx, ry = ((-view.ty) / view.s - minY) * sy;
    const rw = Math.max((vw / view.s) * sx, 6), rh = Math.max((vh / view.s) * sy, 6);
    ctx.strokeStyle = "#e0a23a"; ctx.lineWidth = 1.5;
  ...
  return (
    <canvas ref={ref} width={MW} height={MH}
      ...
      onPointerMove={(e) => { if (e.buttons === 1) { e.stopPropagation(); jump(e); } }}
      style={{
        position: "absolute", bottom: 16, right: 14, width: MW, height: MH,
        background: "rgba(255,255,255,.93)", border: `1px solid ${C.border}`, borderRadius: 10,
        cursor: "crosshair", boxShadow: "0 3px 10px rgba(0,0,0,.10)", touchAction: "none",
      }} />
  );
}
...
function AuthGate({ C, onSignIn }) {
  return (
    <div style={{
      background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10,
      padding: "18px 16px", textAlign: "center", fontSize: 14, lineHeight: 2, color: C.parch,
    }}>
      الشجرة الآن مشتركة ومباشرة ☁<br />
      للتحرير، سجّل الدخول بحساب Google — تعديلاتك تظهر للجميع باسمك ويمكن التراجع عنها من السجل.
      <div>
        <button onClick={onSignIn} style={{
          background: C.gold, color: "#fff", border: "none", padding: "9px 20px", borderRadius: 8,
...
function DateSelect({ C, value, onChange, yearOnly }) {
  const m = String(value || "").match(/^(\d{4})(?:-(\d{1,2}))?(?:-(\d{1,2}))?/);
  const y = m ? +m[1] : "", mo = m && m[2] ? +m[2] : "", d = m && m[3] ? +m[3] : "";
  const nowY = new Date().getFullYear();
  const years = []; for (let i = nowY; i >= 1850; i--) years.push(i);
  const daysIn = (yy, mm) => new Date(yy || 2000, mm, 0).getDate();
  const emit = (ny, nm, nd) => {
    if (!ny) return onChange("");
    let s = String(ny);
    if (nm) { s += "-" + String(nm).padStart(2, "0"); if (nd) s += "-" + String(nd).padStart(2, "0"); }
    onChange(s);
  };
  const sel = {
    flex: 1, minWidth: 0, padding: "8px 6px", borderRadius: 7, border: `1px solid ${C.border}`,
    background: C.panel2, color: C.parch, fontFamily: "'Tajawal'", fontSize: 13.5, outline: "none",
  };
  return (
    <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
      <select value={y} style={sel} onChange={e => {
      ...
      {!yearOnly && (
        <select value={mo} style={sel} disabled={!y} onChange={e => {
          const nm = +e.target.value || "";
          emit(y, nm, nm ? Math.min(d || 0, daysIn(y, nm)) || "" : "");
        }}>
          <option value="">الشهر</option>
          {AR_MONTHS.map((nm, i) => <option key={i} value={i + 1}>{nm}</option>)}
        </select>
      )}
      {!yearOnly && (
        <select value={d} style={{ ...sel, maxWidth: 76 }} disabled={!mo} onChange={e => emit(y, mo, +e.target.value || "")}>
          <option value="">اليوم</option>
          {mo ? Array.from({ length: daysIn(y, mo) }, (_, i) => i + 1).map(dd =>
            <option key={dd} value={dd}>{dd}</option>) : null}
        </select>
      )}
    </div>
  );
...
function VInput({ C, label, value, onChange, field, placeholder, inputMode }) {
  const err = fieldValidators[field] ? fieldValidators[field](value || "") : "";
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ fontSize: 12, color: C.sub, marginBottom: 5, display: "block" }}>{label}</label>
      <input value={value || ""} dir="ltr" inputMode={inputMode} placeholder={placeholder}
        onChange={e => onChange(fieldFilters[field] ? fieldFilters[field](e.target.value) : e.target.value)}
        style={{
          width: "100%", boxSizing: "border-box", padding: "9px 11px", borderRadius: 7,
          border: `1px solid ${err ? "#c0392b" : C.border}`, background: C.panel2, color: C.parch,
          fontFamily: "'Tajawal'", fontSize: 14, outline: "none",
        }} />
      {err && <div style={{ fontSize: 11.5, color: "#c0392b", marginTop: 4 }}>{err}</div>}
    </div>
  );
}
    ...
    <div>
      ...
      {familyMode ? (
        <div>
          ...
          <label style={label}>تاريخ الميلاد (السنة تكفي، الشهر واليوم اختياريان)</label>
          <DateSelect C={C} value={person.dob} onChange={onDob} />
          {person.deceased && (
            <div>
              <label style={label}>سنة الوفاة</label>
              <DateSelect C={C} value={person.dod} onChange={onDod} yearOnly />
            </div>
          )}

          <div style={sect}>بيانات التواصل</div>
          <VInput C={C} field="whatsapp" inputMode="tel" label="واتساب (رقم دولي)"
            placeholder="+9665xxxxxxxx" value={contacts.whatsapp} onChange={v => onContact("whatsapp", v)} />
          <VInput C={C} field="email" inputMode="email" label="البريد الإلكتروني"
            placeholder="name@example.com" value={contacts.email} onChange={v => onContact("email", v)} />
          <VInput C={C} field="linkedin" label="لينكدإن (رابط أو اسم مستخدم)"
            placeholder="linkedin.com/in/…" value={contacts.linkedin} onChange={v => onContact("linkedin", v)} />
          <VInput C={C} field="twitter" label="إكس / تويتر (اسم المستخدم)"
            placeholder="@username" value={contacts.twitter} onChange={v => onContact("twitter", v)} />
...
function OverlayShell({ C, title, onClose, children }) {
  return (
    <div style={{ position: "absolute", inset: 0, background: C.bg, zIndex: 60, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "12px 18px", borderBottom: `1px solid ${C.border}`, flexShrink: 0, background: "#fff" }}>
        <div style={{ fontFamily: "'Amiri',serif", fontSize: 20, color: C.gold, fontWeight: 700 }}>{title}</div>
        <Btn C={C} onClick={onClose}>عودة للشجرة</Btn>
      </div>
...
function StatsOverlay({ C, people, edges, photos, familyMode, onClose }) {
  const sect = { fontSize: 15, color: C.gold, margin: "22px 0 10px", fontWeight: 700, fontFamily: "'Amiri',serif" };
  ...
  const gens = {};
  people.forEach(p => { const g = p.g ?? "؟"; gens[g] = (gens[g] || 0) + 1; });
  const ord = ["الأول","الثاني","الثالث","الرابع","الخامس","السادس","السابع","الثامن","التاسع","العاشر"];
  const genRows = Object.entries(gens).sort((a, b) => (a[0] === "؟") - (b[0] === "؟") || a[0] - b[0]);
  const maxGen = Math.max(...genRows.map(([, n]) => n), 1);

  const living = people.filter(p => !p.deceased && ageOf(p) != null)
    .sort((a, b) => ageOf(b) - ageOf(a)).slice(0, 5);
  ...
  const tile = { background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 8px", textAlign: "center" };
  const num = { fontFamily: "'Amiri',serif", fontSize: 26, fontWeight: 700, color: C.gold };
  const lbl = { fontSize: 12, color: C.sub, marginTop: 2 };

  return (
    <OverlayShell C={C} title="إحصائيات العائلة" onClose={onClose}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(105px, 1fr))", gap: 10 }}>
        <div style={tile}><div style={num}>{people.length}</div><div style={lbl}>فردًا</div></div>
        <div style={tile}><div style={num}>{people.length - deceased.length}</div><div style={lbl}>على قيد الحياة</div></div>
        <div style={tile}><div style={num}>{deceased.length}</div><div style={lbl}>متوفون</div></div>
        <div style={tile}><div style={num}>{withPhoto.length}</div><div style={lbl}>لديهم صور</div></div>
        <div style={tile}><div style={num}>{withDob.length}</div><div style={lbl}>سُجل ميلادهم</div></div>
        {familyMode && <div style={tile}><div style={num}>{males.length} / {females.length}</div><div style={lbl}>ذكور / إناث</div></div>}
      </div>

      <div style={sect}>الأفراد في كل جيل</div>
      {genRows.map(([g, n]) => (
        <div key={g} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 7 }}>
          <div style={{ width: 90, fontSize: 13, color: C.parch, flexShrink: 0 }}>
            {g === "؟" ? "غير محدد" : `الجيل ${ord[g] || +g + 1}`}
          </div>
          <div style={{ flex: 1, background: "#eceadf", borderRadius: 6, height: 20, overflow: "hidden" }}>
            <div style={{ width: `${Math.max(3, n / maxGen * 100)}%`, height: "100%", background: C.goldSoft, borderRadius: 6 }} />
          </div>
          <div style={{ width: 40, fontSize: 13, color: C.sub, textAlign: "left" }}>{n}</div>
        </div>
      ))}

      {familyMode ? (
        <div>
          <div style={sect}>أكبر الأحياء سنًا</div>
          {living.length === 0 && <div style={{ fontSize: 13, color: C.sub }}>لا توجد تواريخ ميلاد مسجلة للأحياء بعد.</div>}
          {living.map(p => (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 10px",
              background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, marginBottom: 6, fontSize: 14 }}>
              <span>{p.name}{p.nickname ? ` (${p.nickname})` : ""}</span>
              <span style={{ color: C.gold, fontWeight: 700 }}>{ageOf(p)} سنة تقريبًا</span>
            </div>
          ))}

          <div style={sect}>أعياد ميلاد خلال ٣٠ يومًا</div>
          {upcoming.length === 0 && <div style={{ fontSize: 13, color: C.sub }}>لا أعياد ميلاد قادمة (تُحسب لمن سُجل شهر ميلادهم).</div>}
          {upcoming.map(p => (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 10px",
              background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, marginBottom: 6, fontSize: 14 }}>
              <span>🎂 {p.name}</span>
              <span style={{ color: C.sub }}>{fmtDate(p.dob)}</span>
            </div>
          ))}
        </div>
      ) : (
        <div style={{ marginTop: 24, fontSize: 13, color: C.sub, background: "#fff",
          border: `1px dashed ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
...
function GalleryOverlay({ C, people, photos, onClose, onOpen }) {
  const withPhoto = people.filter(p => photos[p.id]);
  return (
    <OverlayShell C={C} title={`معرض صور العائلة (${withPhoto.length})`} onClose={onClose}>
      {withPhoto.length === 0 && (
        <div style={{ textAlign: "center", color: C.sub, fontSize: 14, marginTop: 30 }}>لا توجد صور بعد.</div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(108px, 1fr))", gap: 10 }}>
        {withPhoto.map(p => (
          <div key={p.id} onClick={() => onOpen(p)} title={p.name} style={{
            cursor: "pointer", background: "#fff", border: `1px solid ${C.border}`,
            borderRadius: 10, overflow: "hidden", textAlign: "center",
          }}>
            <img src={photos[p.id]} alt={p.name} loading="lazy"
              style={{ width: "100%", height: 108, objectFit: "cover", display: "block" }} />
            <div style={{ fontSize: 11.5, padding: "5px 4px", color: "#2c2415", overflow: "hidden",
              textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "'Tajawal'" }}>
              {p.name}{p.deceased ? " ·" : ""}
            </div>
          </div>
        ))}
      </div>
      <div style={{ textAlign: "center", color: C.sub, fontSize: 12, marginTop: 14 }}>
